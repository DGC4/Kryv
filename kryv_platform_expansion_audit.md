...
## 2026-08-11 Production and Data Baseline

- **Deployment health:** `https://kryv-backend.onrender.com/health` returned `{"status":"ok"}` after the Render free-instance cold start completed. This confirms the API is reachable following the numeric `Me.id` contract hotfix on `main` (`e27a836`).
- **Production database inventory:** Neon project `bold-cake-75596541` includes the existing live-stream, creator engagement, moderation, notification, and Stripe-foundation tables, including `channels`, `stream_sessions`, `viewer_sessions`, `chat_messages`, `clips`, `subscriptions`, `tips`, `creator_payment_accounts`, and `payment_events`.
- **Live-stream safeguard:** The FastPix provisioning and playback implementation has not been modified during this platform-expansion planning pass. All future changes must retain the current channel creation, ingest, preview, and published HLS paths.
- **Crypto provider status:** no current-session connector matched the user-provided `pellicio` name. Provider configuration cannot be activated until the exact provider identity, official API documentation, and a user-supplied credential are available.
- **Public deployment blocker:** the production API responds successfully, but the same Render service currently returns “Frontend not found” at `/`. The server searched the expected `artifacts/blyze/dist` location without finding a bundle. `render.yaml` contains the correct unified build command, so the deployed `kryv-backend` service needs its active Render build configuration and deployment logs reconciled before public-release verification.
- **Render access check:** the available Render dashboard browser session showed no authenticated service controls, so no deployment configuration was altered during the audit.
