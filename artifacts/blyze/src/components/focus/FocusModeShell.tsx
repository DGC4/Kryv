...
import { Clapperboard, Eye, Loader2, MessageCircleMore, Radio, Send, Shield, Users } from 'lucide-react';
...
function FocusChat({ channelId, channelName, requiresFollow, blockedReason }: { channelId: number; channelName: string; requiresFollow: boolean; blockedReason: string | null }) {
  const { user } = useAuthStore();
  const { toast } = useToast();
  const [draft, setDraft] = useState('');
  const messagesQuery = useListChannelMessages(channelId, { query: { enabled: channelId > 0 && !blockedReason, refetchInterval: 15000 } });
...
function LiveFocus({ sourceId, chatEnabled, announcementText }: Omit<FocusModeShellProps, 'sourceType'>) {
  ...
  const chatSettingsQuery = useGetChannelChatSettings(sourceId, { query: { enabled: chatEnabled && sourceId > 0 && !playbackBlockedReason, refetchInterval: 10000 } });
  const playbackId = channel?.playbackAvailable ? (channel.fastpixPlaybackId || channel.playbackId) : null;
