...
export default function KryvPlayer({
  ...
  const [touchFeedback, setTouchFeedback] = useState<"back" | "forward" | null>(
    null,
  );
  ...
  const moveToLiveEdge = useCallback(() => {
    ...
    if (
      typeof hlsLivePosition === "number" &&
      Number.isFinite(hlsLivePosition) &&
      video.duration >= hlsLivePosition
    ) {
  ...
  const handleVideoPointerUp = useCallback(
    (event: React.PointerEvent<HTMLVideoElement>) => {
      ...
      const isDoubleTap = Boolean(
        ...
        Math.abs(event.clientX - previousTap.x) <= 96,
      );
  ...
  return (
    <div
      ...
      aria-busy={!isMediaReady && !hasPlaybackError}
    ...
    >
      ...
      {!isMediaReady && !hasPlaybackError && (
        <div
          className="pointer-events-none absolute inset-0 flex items-center justify-center bg-black/35"
          role="status"
          aria-live="polite"
        >
          <div className="flex items-center gap-3 rounded-xl border border-white/[0.12] bg-black/60 px-4 py-3 text-xs font-bold text-white/80 backdrop-blur">
      ...
      {touchFeedback && (
        <div
          className={`pointer-events-none absolute inset-y-0 ${touchFeedback === "back" ? "left-[16%]" : "right-[16%]"} flex items-center`}
          aria-hidden="true"
        >
      ...
      <div
        className={`pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/45 to-transparent px-3 pb-3 pt-16 transition-opacity duration-200 sm:px-4 sm:pb-4 ${showControls || !isPlaying || showSettings ? "opacity-100" : "opacity-0"}`}
      >
        <div className="pointer-events-auto mx-auto flex max-w-[1800px] flex-col gap-2.5">
          {!live && (
            <input
              ...
              style={
                {
                  "--kryv-player-played": `${playedPercent}%`,
                  "--kryv-player-buffered": `${Math.max(bufferedPercent, playedPercent)}%`,
                } as CSSProperties
              }
            />
          )}
          <div className="flex min-h-11 items-center justify-between gap-2">
            <div className="flex min-w-0 items-center gap-1 sm:gap-1.5">
              <button
                ...
                className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white text-black transition hover:bg-primary hover:text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
              ...
              {!live && (
                <>
                  <button
                    type="button"
                    onClick={() => seekBy(-VOD_SEEK_SECONDS)}
                    className="hidden h-11 w-11 items-center justify-center rounded-full text-white/80 transition hover:bg-white/15 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary sm:inline-flex"
              ...
              <button
                type="button"
                onClick={toggleMute}
                className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-white/80 transition hover:bg-white/15 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
              ...
              <input
                ...
                className="kryv-player__volume hidden w-20 cursor-pointer sm:block"
                style={
                  {
                    "--kryv-volume": `${Math.round((isMuted ? 0 : volume) * 100)}%`,
                  } as CSSProperties
                }
              />
              {live ? (
                <span className="hidden items-center gap-1.5 pl-1 text-[11px] font-semibold text-white/75 sm:inline-flex">
                  <span className="inline-flex h-2 w-2 rounded-full bg-primary shadow-[0_0_10px_hsl(var(--primary)/0.9)]" />
                  Live
                </span>
              ) : (
                <span className="hidden whitespace-nowrap pl-1 font-mono text-[11px] tabular-nums text-white/65 sm:inline">
              ...
              <div className="relative">
                ...
                {showSettings && (
                  <div className="absolute bottom-12 right-0 w-56 rounded-xl border border-white/15 bg-[#0b0d13]/95 p-2 text-sm text-white shadow-2xl backdrop-blur-xl">
                    ...
                    <div className="border-b border-white/[0.08] px-2.5 py-2.5">
                      <div className="mb-1.5 flex items-center justify-between text-[11px] font-semibold text-white/70">
                        ...
                        <span className="text-primary">
                          {selectedQuality === "auto"
                            ? "Auto"
                            : (qualityOptions.find(
                                (option) => option.level === selectedQuality,
                              )?.label ?? "Auto")}
                        </span>
                      </div>
                      <div className="grid grid-cols-3 gap-1">
                        <button
                          ...
                          className={`min-h-9 rounded-md px-1 text-[10px] font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${selectedQuality === "auto" ? "bg-primary text-primary-foreground" : "bg-white/[0.06] text-white/65 hover:bg-white/[0.12] hover:text-white"}`}
                        >
                          Auto
                        </button>
                        {qualityOptions.map((option) => (
                          <button
                            ...
                            className={`min-h-9 rounded-md px-1 text-[10px] font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${selectedQuality === option.level ? "bg-primary text-primary-foreground" : "bg-white/[0.06] text-white/65 hover:bg-white/[0.12] hover:text-white"}`}
                          >
                            {option.label}
                          </button>
                        ))}
                      ...
                      {qualityOptions.length === 0 && (
                        <p className="mt-2 text-[11px] leading-relaxed text-white/50">
                          Manual renditions appear when the source publishes
                          them.
                        </p>
                      )}
                      <p className="mt-2 text-[10px] leading-relaxed text-white/40">
                        Auto adapts only among source renditions that are
                        available for this playback.
                      </p>
                    </div>
                    {!live && (
                      <div className="px-2.5 pb-1 pt-2">
                        <div className="mb-1.5 flex items-center justify-between text-[11px] font-semibold text-white/70">
                          <span>Speed</span>
                          <span className="text-primary">{playbackRate}×</span>
                        </div>
                        <div className="grid grid-cols-5 gap-1">
                          {SPEED_OPTIONS.map((speed) => (
                            <button
                              ...
                              className={`min-h-9 rounded-md text-[10px] font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${playbackRate === speed ? "bg-primary text-primary-foreground" : "bg-white/[0.06] text-white/65 hover:bg-white/[0.12] hover:text-white"}`}
                            >
                              {speed}×
                            </button>
                          ))}
                        ...
                        <button
                          ...
                          className={`mt-2 inline-flex min-h-9 w-full items-center justify-between rounded-lg px-2.5 text-[11px] font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${isLooping ? "bg-primary/18 text-primary" : "bg-white/[0.06] text-white/65 hover:bg-white/[0.12] hover:text-white"}`}
                        >
      ...
      {!isPlaying && !hasPlaybackError && (
        <button
          type="button"
          onClick={togglePlayback}
          className="absolute inset-0 flex items-center justify-center bg-black/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary"
          aria-label="Play video"
        >
          <span className="flex h-14 w-14 items-center justify-center rounded-full border border-white/35 bg-black/55 text-white shadow-xl backdrop-blur transition hover:scale-105 hover:border-primary hover:bg-primary hover:text-primary-foreground sm:h-16 sm:w-16">
      ...
      {hasPlaybackError && (
        <div
          className="absolute inset-0 flex items-center justify-center bg-black/75 px-5 text-center"
          role="alert"
          aria-live="assertive"
        >
          <div className="max-w-sm">
            ...
            <p className="mt-2 text-xs leading-relaxed text-white/60">
              The media source did not load. Retry the player or refresh this
              page in a moment.
            </p>
            <button
              type="button"
              onClick={retryPlayback}
              className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl border border-primary/45 bg-primary/15 px-4 text-sm font-semibold text-primary transition hover:bg-primary hover:text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
            >
              <RotateCcw className="h-4 w-4" /> Retry playback
            </button>
          </div>
        </div>
      )}
