    ...
    <div className="relative z-10 max-w-6xl mx-auto px-4 py-8">
      ...
      {tab === 'analytics' && (
        <section className="space-y-5">
          ...
          </> : <div className="rounded-2xl border border-red-300/20 bg-red-400/[0.05] p-5 text-sm text-red-100">Analytics could not load. Existing owner controls remain available while the API connection is restored.</div>}
        </section>
      )}
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {featureFlags?.map((flag) => {
                ...
                return <article key={flag.key} className={`rounded-2xl border p-5 ${flag.enabled ? 'border-emerald-400/25 bg-emerald-400/[0.045]' : 'border-white/[0.1] bg-black/30'}`}>
                  ...
                  <div className="mt-5 flex items-center justify-between gap-3"><span className="text-[10px] font-bold text-white/35">Updated {new Date(flag.updatedAt).toLocaleString()}</span><Button variant={flag.enabled ? 'secondary' : 'default'} size="sm" disabled={updateFeatureFlag.isPending || (!flag.enabled && ['customer_wallet_custody', 'scheduled_payout_requests', 'provider_withdrawals'].includes(flag.key))} onClick={() => toggleOperationalFlag(flag.key, !flag.enabled)} className="font-black">{updateFeatureFlag.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : flag.enabled ? 'Disable now' : ['customer_wallet_custody', 'scheduled_payout_requests', 'provider_withdrawals'].includes(flag.key) ? 'Locked disabled' : 'Enable'}</Button></div>
                </article>;
              })}
