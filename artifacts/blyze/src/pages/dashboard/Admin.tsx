...
import {
  ...
  Crown, Lock, ShieldAlert, Activity, PlaySquare, Tv, Plus, Clapperboard, CheckCircle2, CircleAlert, FileVideo2, Gavel, History, Send, UploadCloud, Power, Zap, Wallet, Landmark, Clock3, XCircle, Search, Save, CircleCheck, RefreshCw,
} from 'lucide-react';
...
export default function DashboardAdmin() {
  ...
  const [moderationOffset, setModerationOffset] = useState(0);
  const [payoutProfileOffset, setPayoutProfileOffset] = useState(0);
  const [payoutRequestOffset, setPayoutRequestOffset] = useState(0);
  const [creatorBalanceOffset, setCreatorBalanceOffset] = useState(0);
  ...
  const creatorBalancesQuery = useListAdminCreatorBalances({ limit: 50, offset: creatorBalanceOffset }, {
    query: { enabled: me?.role === 'owner' && tab === 'finance', refetchInterval: tab === 'finance' ? 15000 : false },
  });
  const payoutProfilesQuery = useListAdminPayoutProfiles({ limit: 50, offset: payoutProfileOffset }, {
    query: { enabled: me?.role === 'owner' && tab === 'finance' },
  });
  const payoutRequestsQuery = useListAdminPayoutRequests({ limit: 50, offset: payoutRequestOffset }, {
    query: { enabled: me?.role === 'owner' && tab === 'finance' },
  });
  const adsOverviewQuery = useGetAdminAdsOverview({
    query: { enabled: me?.role === 'owner' && tab === 'ads', refetchInterval: tab === 'ads' ? 15000 : false },
  });
  const moderationCasesQuery = useListAdminModerationCases({ status: 'open', limit: 50, offset: moderationOffset }, {
  ...
  const [cinemaMetadataTitle, setCinemaMetadataTitle] = useState('');
  const [cinemaMetadataSynopsis, setCinemaMetadataSynopsis] = useState('');
  const [cinemaMetadataPosterUrl, setCinemaMetadataPosterUrl] = useState('');
  const [cinemaMetadataBackdropUrl, setCinemaMetadataBackdropUrl] = useState('');
  const [cinemaMetadataLogoUrl, setCinemaMetadataLogoUrl] = useState('');
  const [cinemaMetadataContentRating, setCinemaMetadataContentRating] = useState('');
  const [cinemaMetadataReleaseYear, setCinemaMetadataReleaseYear] = useState('');
  const [cinemaMetadataMaturity, setCinemaMetadataMaturity] = useState<'kids' | 'standard' | 'mature'>('standard');
  const [cinemaMetadataRank, setCinemaMetadataRank] = useState('0');
  ...
  React.useEffect(() => {
    ...
    if (!title) return;
    setCinemaMetadataTitle(title.title);
    setCinemaMetadataSynopsis(title.synopsis ?? '');
    setCinemaMetadataPosterUrl(title.posterUrl ?? '');
    setCinemaMetadataBackdropUrl(title.backdropUrl ?? '');
    setCinemaMetadataLogoUrl(title.logoUrl ?? '');
    setCinemaMetadataContentRating(title.contentRating ?? '');
    setCinemaMetadataReleaseYear(title.releaseYear === null ? '' : String(title.releaseYear));
    setCinemaMetadataMaturity(title.maturityLevel);
    setCinemaMetadataRank(String(title.editorialRank));
  }, [cinemaDetailQuery.data?.id, cinemaDetailQuery.data?.updatedAt]);
  ...
  const saveCinemaMetadata = (event: React.FormEvent) => {
    event.preventDefault();
    if (selectedCinemaTitleId === null || !cinemaMetadataTitle.trim()) return;
    const editorialRank = Number(cinemaMetadataRank);
    const releaseYear = cinemaMetadataReleaseYear.trim() ? Number(cinemaMetadataReleaseYear) : null;
    if (!Number.isInteger(editorialRank) || editorialRank < -100000 || editorialRank > 100000) {
      toast({ title: 'Editorial rank is invalid', description: 'Use a whole number between -100000 and 100000.', variant: 'destructive' });
      return;
    }
    if (releaseYear !== null && (!Number.isInteger(releaseYear) || releaseYear < 1888 || releaseYear > 2200)) {
      toast({ title: 'Release year is invalid', description: 'Use a whole year between 1888 and 2200, or leave it blank.', variant: 'destructive' });
      return;
    }
    updateCinemaTitle.mutate({ id: selectedCinemaTitleId, data: {
      title: cinemaMetadataTitle.trim(),
      synopsis: cinemaMetadataSynopsis.trim() || null,
      posterUrl: cinemaMetadataPosterUrl.trim() || null,
      backdropUrl: cinemaMetadataBackdropUrl.trim() || null,
      logoUrl: cinemaMetadataLogoUrl.trim() || null,
      contentRating: cinemaMetadataContentRating.trim() || null,
      releaseYear,
      maturityLevel: cinemaMetadataMaturity,
      editorialRank,
      reason: 'Owner updated title metadata',
    } }, {
      onSuccess: () => { refreshCinemaDesk(); toast({ title: 'Cinema metadata saved', description: 'The owner-controlled title record and public catalog metadata were refreshed.' }); },
      onError: (err: any) => toast({ title: 'Cinema metadata could not be saved', description: err?.body?.error || err?.message || 'Review the title metadata values.', variant: 'destructive' }),
    });
  };
      ...
      <div className="flex items-start gap-4 mb-8">
        ...
        <div className="flex-1">
          <div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:gap-3">
            <h1 className="text-2xl font-display font-black tracking-tight text-white sm:text-3xl">Owner Console</h1>
            <div className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-yellow-400/20 bg-yellow-400/10 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.12em] text-yellow-400 sm:px-3 sm:text-[10px] sm:tracking-widest">
              <GoldenDBadge className="h-2.5 w-2.5 shrink-0" />
              <span className="truncate">FanoDGC · Permanent Owner</span>
            </div>
          </div>
...
{statsLoading ? (
      ...
      <div className="-mx-4 mb-5 flex snap-x snap-mandatory gap-1 overflow-x-auto overscroll-x-contain border-b border-white/[0.08] px-4 [scroll-padding-inline:1rem] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mx-0 sm:px-0">
        {(['overview', 'analytics', 'users', 'channels', 'videos', 'cinema', 'finance', 'ads', 'safety', 'operations', 'focus'] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`min-h-11 shrink-0 snap-start px-4 text-sm font-semibold capitalize border-b-2 transition-colors sm:font-bold ${
              tab === t
      ...
      {tab === 'analytics' && (
        <section className="space-y-5">
          <div className="rounded-2xl border border-primary/20 bg-[radial-gradient(circle_at_top_right,rgba(51,238,196,0.14),transparent_38%),linear-gradient(130deg,rgba(91,70,255,0.12),rgba(8,10,15,0.96)_58%)] p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2 text-primary"><Activity className="h-5 w-5" /><h2 className="text-lg font-black text-white">Connected platform analytics</h2></div><p className="mt-2 max-w-3xl text-sm leading-relaxed text-white/55">Stream sessions, recorded chat activity, active memberships, top live creators, recent Watch-history records, and immutable revenue movements. This view rechecks the record-derived snapshot through REST every 30 seconds while open; it does not create estimates for watch time, conversion, total plays, or fiat value.</p></div><div className="flex w-full gap-2 sm:w-auto"><div className="flex flex-1 rounded-xl border border-white/[0.1] bg-black/25 p-1 sm:flex-none">{([1, 7, 30] as const).map((days) => <button key={days} type="button" onClick={() => setAnalyticsRangeDays(days)} className={`min-h-10 flex-1 rounded-lg px-3 text-xs font-black transition sm:flex-none ${analyticsRangeDays === days ? 'bg-primary text-primary-foreground' : 'text-white/45 hover:text-white'}`}>{days === 1 ? '24h' : `${days}d`}</button>)}</div><Button type="button" variant="secondary" size="sm" onClick={() => { void adminAnalyticsQuery.refetch(); }} disabled={adminAnalyticsQuery.isFetching} className="min-h-10 shrink-0 border border-white/[0.1] bg-black/25 px-3 text-xs font-black text-white/70 hover:text-primary disabled:cursor-wait" aria-label="Recheck recorded analytics"><RefreshCw className={`h-3.5 w-3.5 ${adminAnalyticsQuery.isFetching ? 'animate-spin' : ''}`} /><span className="ml-1.5 hidden sm:inline">Recheck</span></Button></div></div></div>
          {adminAnalyticsQuery.isLoading ? <div className="flex justify-center py-16"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div> : adminAnalyticsQuery.data ? <><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5"><StatCard label="Streams" value={adminAnalyticsQuery.data.summary.streamSessions} icon={Radio} accent /><div className="rounded-xl border border-white/[0.08] bg-black/40 p-4"><p className="text-2xl font-display font-black leading-none text-white">{formatDuration(adminAnalyticsQuery.data.summary.streamSeconds)}</p><p className="mt-1 text-xs text-white/40">Time live</p></div><StatCard label="Chat messages" value={adminAnalyticsQuery.data.summary.chatMessages} icon={Activity} /><StatCard label="Active creators" value={adminAnalyticsQuery.data.summary.activeCreators} icon={Users} /><StatCard label="Active memberships" value={adminAnalyticsQuery.data.summary.activeSubscriptions} icon={Crown} /></div>
            ...
            <article className="overflow-hidden rounded-2xl border border-white/[0.08] bg-black/25"><div className="border-b border-white/[0.07] px-5 py-4"><h3 className="text-sm font-black text-white">Top Watch titles by recorded activity</h3><p className="mt-1 text-xs leading-relaxed text-white/40">Ranked by Watch-history rows whose latest recorded activity falls inside the selected range. These are not total-play counts, all-time unique viewers, or watch-time estimates.</p></div><div className="divide-y divide-white/[0.07]">{adminAnalyticsQuery.data.topWatchVideos.length ? adminAnalyticsQuery.data.topWatchVideos.map((video, index) => <div key={video.videoId} className="flex items-center gap-3 p-4"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-xs font-black text-primary">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-white">{video.videoTitle}</p><p className="mt-1 truncate text-[10px] text-white/40">{video.channelDisplayName}</p></div><span className="shrink-0 text-right"><b className="block text-sm font-black text-primary">{video.recordedViewerCount}</b><span className="block text-[9px] font-bold uppercase tracking-wider text-white/35">records</span></span></div>) : <p className="p-6 text-sm text-white/40">No Watch-history records fall within this range.</p>}</div></article>
          ...
          {financeOverviewQuery.isLoading ? <div className="flex justify-center py-16"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div> : financeOverviewQuery.isError ? <div className="rounded-2xl border border-red-300/20 bg-red-400/[0.05] p-5 text-sm text-red-100"><p className="font-black">Finance overview is temporarily unavailable</p><p className="mt-1 leading-relaxed text-red-100/70">Kryv cannot safely present creator liabilities, payout queue totals, or provider configuration state while the authoritative overview is unavailable.</p><Button type="button" variant="outline" onClick={() => financeOverviewQuery.refetch()} className="mt-4 border-red-200/25 bg-red-200/[0.08] font-black text-red-50 hover:bg-red-200/[0.14] hover:text-red-50">Retry Finance Command</Button></div> : (
            <>
              ...
              <section className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
                <article className="overflow-hidden rounded-2xl border border-white/[0.08] bg-black/25"><div className="border-b border-white/[0.07] p-5"><div className="flex items-start justify-between gap-4"><div><h3 className="text-sm font-black text-white">Creator balance registry</h3><p className="mt-1 text-xs leading-relaxed text-white/40">Current balances grouped by creator and asset. Select a row to inspect the corresponding immutable movement history.</p></div><span className="shrink-0 rounded-full border border-white/[0.08] bg-white/[0.03] px-2.5 py-1 text-[9px] font-black uppercase text-white/45">{creatorBalancesQuery.data?.total ?? 0} assets</span></div></div><div className="max-h-[31rem] divide-y divide-white/[0.06] overflow-y-auto p-2">{creatorBalancesQuery.isLoading ? <div className="flex justify-center py-10"><Loader2 className="h-5 w-5 animate-spin text-primary" /></div> : creatorBalancesQuery.data?.items.length ? creatorBalancesQuery.data.items.map((balance) => <button key={`${balance.channelId}-${balance.currency}`} type="button" onClick={() => setSelectedCreatorBalanceChannelId(balance.channelId)} aria-pressed={selectedCreatorBalanceChannelId === balance.channelId} className={`w-full rounded-xl p-3 text-left transition ${selectedCreatorBalanceChannelId === balance.channelId ? 'bg-primary/10 ring-1 ring-primary/35' : 'hover:bg-white/[0.045]'}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-bold text-white">{balance.channelDisplayName}</p><p className="mt-1 truncate text-[10px] text-white/35">@{balance.creatorUsername} · {balance.currency}</p></div><span className="shrink-0 text-xs font-black text-primary">{balance.availableAmount}</span></div><div className="mt-2 flex gap-3 text-[10px] text-white/45"><span>Pending {balance.pendingAmount}</span><span>Held {balance.heldAmount}</span></div></button>) : <p className="m-3 rounded-xl border border-dashed border-white/[0.1] p-4 text-xs leading-relaxed text-white/35">No current creator balance records have been created from settled ledger activity.</p>}</div><nav className="flex flex-wrap items-center justify-between gap-2 border-t border-white/[0.07] p-3" aria-label="Creator balance registry pages"><Button type="button" variant="outline" size="sm" disabled={creatorBalanceOffset === 0} onClick={() => setCreatorBalanceOffset((current) => Math.max(0, current - 50))}>Newer assets</Button><span className="text-[10px] text-white/45">{creatorBalanceOffset + 1}–{Math.min(creatorBalanceOffset + (creatorBalancesQuery.data?.items.length ?? 0), creatorBalancesQuery.data?.total ?? 0)} of {creatorBalancesQuery.data?.total ?? 0}</span><Button type="button" variant="outline" size="sm" disabled={creatorBalanceOffset + (creatorBalancesQuery.data?.items.length ?? 0) >= (creatorBalancesQuery.data?.total ?? 0)} onClick={() => setCreatorBalanceOffset((current) => current + 50)}>Older assets</Button></nav></article>
      ...
      {tab === 'users' && (
        ...
        <div className="space-y-3 md:hidden">{userRegistryQuery.isLoading ? <div className="flex justify-center rounded-xl border border-white/[0.08] bg-black/30 p-8"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : userRegistryQuery.data?.items.length ? userRegistryQuery.data.items.map((u) => <article key={u.id} className="rounded-2xl border border-white/[0.08] bg-black/30 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2">{u.role === 'owner' && <GoldenDBadge className="h-3.5 w-3.5" />}<p className="truncate text-sm font-black text-white">{u.username}</p></div><p className="mt-1 truncate font-mono text-[10px] text-white/35">User #{u.id}</p></div><span className={`shrink-0 rounded-md border px-2 py-1 text-[9px] font-black uppercase tracking-wider ${u.role === 'owner' ? 'border-primary/20 bg-primary/15 text-primary' : 'border-white/[0.08] bg-white/[0.06] text-white/50'}`}>{u.role}</span></div><div className="mt-3 grid grid-cols-2 gap-2 rounded-xl border border-white/[0.07] bg-white/[0.02] p-3 text-xs"><div><p className="text-[10px] font-semibold text-white/35">Security</p><p className={`mt-1 inline-flex items-center gap-1 font-black ${u.banned ? 'text-red-300' : 'text-emerald-200'}`}>{u.banned ? <ShieldAlert className="h-3.5 w-3.5" /> : <ShieldCheck className="h-3.5 w-3.5" />}{u.banned ? 'Banned' : 'Secure'}</p></div><div><p className="text-[10px] font-semibold text-white/35">Joined</p><p className="mt-1 font-bold text-white/75">{new Date(u.createdAt).toLocaleDateString()}</p></div></div><div className="mt-3 grid grid-cols-2 gap-2"><Button type="button" variant="secondary" onClick={() => setSelectedUserId(u.id)} className="min-h-11 text-xs font-black"><Eye className="mr-1.5 h-3.5 w-3.5" />Activity</Button>{u.role !== 'owner' ? <Button type="button" variant={u.banned ? 'secondary' : 'destructive'} onClick={() => toggleBan(String(u.id), u.banned)} disabled={updateUser.isPending} className="min-h-11 text-xs font-black">{u.banned ? <><ShieldCheck className="mr-1.5 h-3.5 w-3.5" />Unban</> : <><Ban className="mr-1.5 h-3.5 w-3.5" />Ban</>}</Button> : <div className="flex min-h-11 items-center justify-center rounded-xl border border-primary/15 bg-primary/[0.04] text-[10px] font-black uppercase tracking-wider text-primary/65"><Lock className="mr-1.5 h-3.5 w-3.5" />Protected</div>}</div></article>) : <div className="rounded-xl border border-dashed border-white/[0.12] bg-black/20 p-6 text-center text-sm text-white/40">{normalizedUserSearch ? 'No user records match this search.' : 'No users yet.'}</div>}</div>
        <div className="hidden overflow-x-auto rounded-xl border border-white/[0.08] bg-black/30 backdrop-blur md:block">
      ...
      {/* Channels table */}
      {tab === 'channels' && (
        <div className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-black text-white">Creator channels</h2><p className="mt-1 text-xs text-white/40">Search active and offline creator channels, open public profiles, or use existing owner removal controls.</p></div><label className="relative block sm:w-72"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/35" /><input value={adminChannelSearch} onChange={(event) => { setAdminChannelSearch(event.target.value); setAdminChannelOffset(0); }} maxLength={80} placeholder="Search channel or slug" className="h-11 w-full rounded-xl border border-white/[0.1] bg-black/30 py-2 pl-9 pr-3 text-sm text-white outline-none focus:border-primary/60" /></label></div><div className="space-y-3 md:hidden">{channelRegistryQuery.isLoading ? <div className="flex justify-center rounded-xl border border-white/[0.08] bg-black/30 p-8"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : channelRegistryQuery.data?.items.length ? channelRegistryQuery.data.items.map((c) => <article key={c.id} className="rounded-2xl border border-white/[0.08] bg-black/30 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><Link href={`/profile/${c.slug}`} className="block truncate text-sm font-black text-white transition hover:text-primary">{c.displayName}</Link><p className="mt-1 truncate font-mono text-[10px] text-white/35">/{c.slug}</p></div><span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-black uppercase ${c.isLive ? 'bg-red-400/10 text-red-200' : 'bg-white/[0.07] text-white/45'}`}>{c.isLive ? 'Live' : 'Offline'}</span></div><div className="mt-3 grid grid-cols-2 gap-2 rounded-xl border border-white/[0.07] bg-white/[0.02] p-3 text-xs"><div><p className="text-[10px] font-semibold text-white/35">Category</p><p className="mt-1 font-bold text-white/75">{c.categoryName ?? 'Uncategorized'}</p></div><div><p className="text-[10px] font-semibold text-white/35">Followers</p><p className="mt-1 font-bold text-white/75">{c.followerCount.toLocaleString()}</p></div></div><div className="mt-3 grid grid-cols-2 gap-2"><Link href={`/profile/${c.slug}`} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/[0.1] bg-white/[0.03] px-3 text-xs font-black text-white transition hover:border-primary/45 hover:text-primary"><Eye className="mr-1.5 h-3.5 w-3.5" />Profile</Link><Button type="button" variant="destructive" onClick={() => removeChannel(c.id)} disabled={deleteChannel.isPending} className="min-h-11 text-xs font-black"><Trash2 className="mr-1.5 h-3.5 w-3.5" />Remove</Button></div></article>) : <div className="rounded-xl border border-dashed border-white/[0.12] bg-black/20 p-6 text-center text-sm text-white/40">{normalizedChannelSearch ? 'No channels match this search.' : 'No channels yet.'}</div>}</div><div className="hidden overflow-x-auto rounded-xl border border-white/[0.08] bg-black/30 backdrop-blur md:block">
          ...
          <div className="grid gap-5 xl:grid-cols-[0.82fr_1.45fr]">
            ...
            <section className="min-h-[430px] overflow-hidden rounded-2xl border border-white/[0.08] bg-black/30">
              {cinemaDetailQuery.isLoading ? <div className="flex min-h-[430px] items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-primary" /></div> : cinemaDetailQuery.data ? (() => { const title = cinemaDetailQuery.data; return <div>
                ...
                <form onSubmit={saveCinemaMetadata} className="border-b border-white/[0.08] bg-black/15 p-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><h4 className="text-sm font-black text-white">Catalog metadata</h4><p className="mt-1 text-[11px] leading-relaxed text-white/45">Owner-only public title copy and artwork. Rights windows, approved assets, and release readiness remain separate safeguards.</p></div><Button type="submit" size="sm" disabled={updateCinemaTitle.isPending || !cinemaMetadataTitle.trim()} className="h-9 shrink-0 text-[10px] font-black uppercase tracking-wider">{updateCinemaTitle.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <><Save className="mr-1.5 h-3.5 w-3.5" /> Save metadata</>}</Button></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-xs font-bold text-white/65">Title<input value={cinemaMetadataTitle} onChange={event => setCinemaMetadataTitle(event.target.value)} maxLength={160} className="mt-1.5 h-9 w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 text-xs text-white outline-none focus:border-primary/60" /></label><label className="text-xs font-bold text-white/65">Maturity<select value={cinemaMetadataMaturity} onChange={event => setCinemaMetadataMaturity(event.target.value as typeof cinemaMetadataMaturity)} className="mt-1.5 h-9 w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 text-xs text-white outline-none focus:border-primary/60"><option value="kids">Kids</option><option value="standard">Standard</option><option value="mature">Mature</option></select></label><label className="text-xs font-bold text-white/65">Poster URL<input value={cinemaMetadataPosterUrl} onChange={event => setCinemaMetadataPosterUrl(event.target.value)} maxLength={2048} placeholder="https://…" className="mt-1.5 h-9 w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 text-xs text-white outline-none focus:border-primary/60" /></label><label className="text-xs font-bold text-white/65">Backdrop URL<input value={cinemaMetadataBackdropUrl} onChange={event => setCinemaMetadataBackdropUrl(event.target.value)} maxLength={2048} placeholder="https://…" className="mt-1.5 h-9 w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 text-xs text-white outline-none focus:border-primary/60" /></label><label className="text-xs font-bold text-white/65">Logo URL<input value={cinemaMetadataLogoUrl} onChange={event => setCinemaMetadataLogoUrl(event.target.value)} maxLength={2048} placeholder="https://…" className="mt-1.5 h-9 w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 text-xs text-white outline-none focus:border-primary/60" /></label><label className="text-xs font-bold text-white/65">Content rating<input value={cinemaMetadataContentRating} onChange={event => setCinemaMetadataContentRating(event.target.value)} maxLength={32} placeholder="PG-13" className="mt-1.5 h-9 w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 text-xs text-white outline-none focus:border-primary/60" /></label><label className="text-xs font-bold text-white/65">Release year<input type="number" value={cinemaMetadataReleaseYear} onChange={event => setCinemaMetadataReleaseYear(event.target.value)} min={1888} max={2200} placeholder="2026" className="mt-1.5 h-9 w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 text-xs text-white outline-none focus:border-primary/60" /></label><label className="text-xs font-bold text-white/65">Editorial rank<input type="number" value={cinemaMetadataRank} onChange={event => setCinemaMetadataRank(event.target.value)} min={-100000} max={100000} className="mt-1.5 h-9 w-full rounded-lg border border-white/[0.1] bg-black/30 px-3 text-xs text-white outline-none focus:border-primary/60" /></label><label className="text-xs font-bold text-white/65 sm:col-span-2">Synopsis<textarea value={cinemaMetadataSynopsis} onChange={event => setCinemaMetadataSynopsis(event.target.value)} maxLength={5000} rows={3} className="mt-1.5 w-full resize-y rounded-lg border border-white/[0.1] bg-black/30 px-3 py-2 text-xs text-white outline-none focus:border-primary/60" placeholder="Owner-approved public synopsis" /></label></div></form>
