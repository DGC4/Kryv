...
import type { VideoSummary, ViewerProfile } from '@workspace/api-client-react';
...
function CinemaTitleCard({ video, index }: { video: VideoSummary; index: number }) {
  const assetReady = video.uploadStatus === 'ready' && Boolean(video.playbackId);
  return (
    <Link href={`/cinema/${video.id}`} className="group relative w-[142px] shrink-0 snap-start sm:w-[174px] lg:w-[196px]">
      <article className="relative aspect-[2/3] overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.04] shadow-lg transition-all duration-300 group-hover:z-10 group-hover:-translate-y-2 group-hover:scale-[1.04] group-hover:border-primary/55 group-hover:shadow-2xl group-focus-visible:z-10 group-focus-visible:-translate-y-2 group-focus-visible:scale-[1.04]">
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 transition-transform duration-300 group-hover:translate-y-0 sm:translate-y-6"><h3 className="truncate text-sm font-black text-white">{video.title}</h3><p className="mt-1 truncate text-[10px] font-semibold text-white/65">{video.categoryName || 'Kryv Cinema'} · {formatRuntime(video.durationSeconds)}</p><div className="mt-3 flex items-center gap-2 opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100"><span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-white text-black"><Play className="h-3.5 w-3.5 fill-current" /></span><span className="text-[10px] font-black text-white">View title</span></div></div>
      </article>
    </Link>
  );
}
...
export default function CinemaHome() {
  ...
  const heroReady = hero?.uploadStatus === 'ready' && Boolean(hero.playbackId);
    ...
    <div className="relative z-10 overflow-hidden pb-16 sm:pb-24">
      <section className="relative overflow-hidden border-b border-white/[0.06] bg-gradient-to-br from-indigo-500/[0.16] via-black to-primary/[0.10]">
        ...
        <div className="relative mx-auto flex min-h-[390px] max-w-[1600px] flex-col justify-end px-4 py-10 sm:min-h-[500px] sm:px-6 sm:py-14 lg:px-8"><div className="absolute right-4 top-5 z-10 sm:right-6 sm:top-7 lg:right-8">{activeProfile && <button type="button" onClick={() => setActiveProfileId(null)} className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/35 px-3 py-1.5 text-xs font-bold text-white/80 backdrop-blur transition-colors hover:bg-white/15"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-primary/20 text-[10px] text-primary">{activeProfile.name.slice(0, 1).toUpperCase()}</span>{activeProfile.name}</button>}</div>
          <div className="max-w-2xl"><div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/35 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-white/80 backdrop-blur"><Clapperboard className="h-3.5 w-3.5 text-primary" /> Kryv Cinema</div><h1 className="mt-4 text-4xl font-black tracking-tight text-white sm:text-5xl lg:text-6xl">{hero?.title || 'Kryv Cinema'}</h1><p className="mt-3 max-w-xl text-sm leading-relaxed text-white/65 sm:text-base">{hero?.description || 'A carefully governed catalog for original and licensed stories, with title access tied to publishing and rights status.'}</p>{hero && <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-bold text-white/65"><span>{hero.categoryName || 'Kryv Cinema'}</span><span>{formatRuntime(hero.durationSeconds)}</span><span className={`inline-flex items-center gap-1.5 ${heroReady ? 'text-emerald-200' : 'text-white/55'}`}><LockKeyhole className="h-3.5 w-3.5" /> {heroReady ? 'Access determined on title page' : 'Catalog preview'}</span></div>}<div className="mt-6 flex flex-wrap gap-3">{hero && <Link href={`/cinema/${hero.id}`}><Button className="h-11 rounded-xl bg-white px-5 text-sm font-black text-black hover:bg-white/90"><Info className="mr-2 h-4 w-4" /> Title details</Button></Link>}<span className="inline-flex h-11 items-center gap-2 rounded-xl border border-white/20 bg-black/25 px-4 text-xs font-bold text-white/70 backdrop-blur"><LockKeyhole className="h-3.5 w-3.5 text-primary" /> Rights-aware catalog</span></div></div>
        </div>
      </section>

      <div className="mx-auto max-w-[1600px] space-y-10 px-4 py-8 sm:space-y-14 sm:px-6 sm:py-10 lg:px-8">
        {hasTitles ? rows.map((row, index) => <section key={`${row.title}-${index}`}><div className="mb-4 flex items-end justify-between gap-4"><div><div className="flex items-center gap-2 text-primary"><Sparkles className="h-4 w-4" /><span className="text-[11px] font-black uppercase tracking-[0.18em]">Cinema collection</span></div><h2 className="mt-1 text-xl font-black text-white sm:text-2xl">{row.title}</h2><p className="mt-1 text-xs text-white/40">Hover or focus a title for its real catalog metadata. Trailer playback only appears after a rights-cleared trailer asset is published.</p></div></div>{row.items.length > 0 ? <RowScroller>{row.items.map((video, itemIndex) => <CinemaTitleCard key={video.id} video={video} index={itemIndex} />)}</RowScroller> : <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-6 text-sm text-white/40">This collection will appear here when titles are announced.</div>}</section>) : <section className="rounded-2xl border border-white/[0.08] bg-gradient-to-br from-white/[0.04] to-indigo-500/[0.07] p-6 sm:p-8"><div className="flex max-w-2xl flex-col gap-3"><div className="flex items-center gap-2 text-primary"><Clapperboard className="h-5 w-5" /><span className="text-[11px] font-black uppercase tracking-[0.18em]">Cinema control room</span></div><h2 className="text-2xl font-black text-white">The Cinema catalog is ready for its first approved title.</h2><p className="text-sm leading-relaxed text-white/55">Title artwork, runtime, trailers, original media, and rights windows are published through the owner workflow. Nothing is made playable merely because an image exists.</p></div></section>}

        <section aria-labelledby="cinema-genres"><div className="mb-4"><div className="flex items-center gap-2 text-primary"><Sparkles className="h-4 w-4" /><span className="text-[11px] font-black uppercase tracking-[0.18em]">Collection map</span></div><h2 id="cinema-genres" className="mt-1 text-2xl font-black text-white sm:text-3xl">Explore the catalog by genre</h2><p className="mt-1 text-sm text-white/45">Genre shelves will populate only from published title metadata.</p></div>{genres && genres.length > 0 ? <div className="-mx-4 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-3 sm:mx-0 sm:grid sm:snap-none sm:grid-cols-3 sm:gap-4 sm:overflow-visible sm:px-0 lg:grid-cols-6">{genres.map((genre, index) => <article key={genre.id} className="relative aspect-[4/5] w-[42vw] shrink-0 snap-start overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.04] p-3 sm:w-auto sm:shrink sm:p-4">{genre.imageUrl ? <img src={genre.imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover opacity-70" /> : <div className={`absolute inset-0 bg-gradient-to-br ${GENRE_THEMES[index % GENRE_THEMES.length]}`} />}<div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent" /><div className="relative flex h-full flex-col justify-between"><span className="w-fit rounded-full border border-white/20 bg-black/25 px-2 py-1 text-[9px] font-black uppercase tracking-[0.12em] text-white/80 backdrop-blur">Catalog</span><div><Clapperboard className="mb-2 h-5 w-5 text-white/75" /><h3 className="text-sm font-black text-white sm:text-base">{genre.name}</h3><p className="mt-1 text-[11px] text-white/65">Title shelf</p></div></div></article>)}</div> : <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] p-6 text-sm text-white/45">Genre collections are being organized.</div>}</section>
      </div>
