      ...
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
        ...
        <div className="mt-7 grid gap-8 lg:grid-cols-[minmax(0,1fr)_260px]">
          <article>
            ...
            {title.trailerPlaybackId && <div className="mt-6 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary text-primary-foreground"><Play className="h-4 w-4 fill-current" /></span><div><p className="text-sm font-black text-white">Trailer available</p><p className="text-xs text-white/45">This preview is an owner-approved Cinema asset.</p></div></div><Button type="button" variant="secondary" size="sm" onClick={() => setShowTrailer(value => !value)}>{showTrailer ? 'Hide trailer' : 'Watch trailer'}</Button></div>{showTrailer && <div className="mt-4 aspect-video overflow-hidden rounded-xl border border-white/[0.08] bg-black"><KryvPlayer src={`https://stream.fastpix.com/${title.trailerPlaybackId}.m3u8`} poster={title.backdropUrl || title.posterUrl || undefined} className="h-full w-full object-contain" ariaLabel={`${title.title} trailer player`} /></div>}</div>}
