...
import { ArrowLeft, Clapperboard, Info, Loader2, LockKeyhole, Play, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="relative z-10 min-h-screen overflow-hidden bg-black text-white">
      <section className="relative overflow-hidden border-b border-white/[0.06]">
        ...
        <Link href="/cinema" className="absolute left-4 top-5 z-20 sm:left-6 sm:top-7"><span className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 bg-black/40 text-white/75 backdrop-blur transition-colors hover:bg-white/15 hover:text-white"><ArrowLeft className="h-5 w-5" /></span></Link>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8"><div className="relative aspect-video overflow-hidden rounded-2xl border border-white/10 bg-black shadow-2xl"><HlsPlayer src={`https://stream.fastpix.com/${title.featurePlaybackId}.m3u8`} poster={title.backdropUrl || title.posterUrl || undefined} className="h-full w-full" /></div><div className="mt-7 grid gap-8 lg:grid-cols-[1fr_260px]"><article><div className="flex items-center gap-2 text-primary"><Info className="h-4 w-4" /><span className="text-[11px] font-black uppercase tracking-[0.18em]">About this title</span></div><p className="mt-3 text-base leading-relaxed text-white/75">{title.synopsis || 'Title details will be added by the Cinema publishing team.'}</p>{title.trailerPlaybackId && <div className="mt-6 rounded-xl border border-white/[0.08] bg-white/[0.03] p-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary text-primary-foreground"><Play className="h-4 w-4 fill-current" /></span><div><p className="text-sm font-black text-white">Trailer available</p><p className="text-xs text-white/45">The owner publishing desk has approved a trailer asset for this release.</p></div></div></div>}</article><aside className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-white/35">Viewing access</p><p className="mt-3 text-sm font-black text-white">{entitlementLabel(title.entitlementType)}</p><p className="mt-2 text-xs leading-relaxed text-white/45">Availability is controlled by the title’s active publishing and rights settings.</p><div className="mt-5 border-t border-white/[0.08] pt-4"><div className="flex items-center gap-2 text-xs font-semibold text-emerald-200"><LockKeyhole className="h-3.5 w-3.5" /> Approved release</div></div></aside></div></main>
    </div>
  );
}
