  ...
  return (
    <div className="relative z-10 min-h-screen overflow-hidden bg-black text-white">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
        {title.playbackAvailable && title.featurePlaybackId ? <div className="relative aspect-video overflow-hidden rounded-2xl border border-white/10 bg-black shadow-2xl sm:rounded-3xl"><HlsPlayer src={`https://stream.fastpix.com/${title.featurePlaybackId}.m3u8`} poster={title.backdropUrl || title.posterUrl || undefined} className="h-full w-full" /></div> : <div className="relative flex aspect-video flex-col items-center justify-center overflow-hidden rounded-2xl border border-amber-300/20 bg-[#050505] px-5 text-center shadow-2xl sm:rounded-3xl"><LockKeyhole className="h-8 w-8 text-amber-100/80" /><p className="mt-4 text-lg font-bold text-white">Viewing access is not available yet</p><p className="mt-2 max-w-lg text-sm leading-relaxed text-white/55">{title.playbackBlockedReason || 'This title is visible in the owner-published catalog, but playback is not currently available.'}</p><span className="mt-5 inline-flex rounded-full border border-white/[0.12] bg-black/25 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-white/60">{entitlementLabel(title.entitlementType)}</span></div>}
