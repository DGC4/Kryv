...
export default function LiveHome() {
  ...
  const { data: followedLive, isLoading: followedLiveLoading, isError: followedLiveError, refetch: refetchFollowedLive } = useListFollowedLiveChannels({
    ...
    <div className="relative z-10 overflow-hidden">
      <div className="mx-auto max-w-[1600px] space-y-10 px-4 py-5 sm:space-y-14 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
        ...
        <section aria-labelledby="live-now">
          ...
          {liveFeed === 'following' && followedLiveLoading ? <div className="flex min-h-40 items-center justify-center rounded-2xl border border-white/[0.08] bg-white/[0.02]"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : liveFeed === 'following' && followedLiveError ? <div className="flex flex-col items-start justify-between gap-5 rounded-2xl border border-red-300/20 bg-red-400/[0.05] p-5 sm:flex-row sm:items-center sm:p-7"><div><h3 className="text-lg font-black text-red-100">Your followed-live feed is temporarily unavailable</h3><p className="mt-1 text-sm text-red-100/70">Kryv cannot confirm whether followed creators are live right now.</p></div><button type="button" onClick={() => refetchFollowedLive()} className="shrink-0 inline-flex min-h-10 items-center rounded-xl border border-red-200/25 bg-red-200/[0.08] px-4 text-sm font-black text-red-50 transition hover:bg-red-200/[0.14]">Retry following</button></div> : visibleLiveChannels.length > 0 ? (
            <div className="-mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-3 sm:mx-0 sm:grid sm:snap-none sm:grid-cols-2 sm:gap-x-5 sm:gap-y-7 sm:overflow-visible sm:px-0 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
